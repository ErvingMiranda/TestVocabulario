...
- Create the action class in the `org.Ezone.POO.TestVocabulario.actions` package, typically extending `ViewBaseAction`
...
<controller name="YourFirstEntity">
    ...
    <action name="showAmount" mode="detail"
        class="org.Ezone.POO.TestVocabulario.actions.ShowAmountAction"/>
</controller>
```
...
- ✅ Place action classes under `org.Ezone.POO.TestVocabulario.actions`
...
### Basic Test Example
```java
package org.Ezone.POO.TestVocabulario.tests;
...
- Create a `dashboards` package (e.g., `org.Ezone.POO.TestVocabulario.dashboards`) and place the `Dashboard` class and its auxiliary classes in it.
